...
const ParentListPage = async ({
  ...
  const columns = [
    ...
    {
      header: "Öğrenci İsmi & Soyisim (ID)",
      accessor: "students",
      ...
      <td className="hidden md:table-cell">
        <div className="flex flex-col">
          {item.students.map((student) => {
            return (
              <div className="text-md font-semibold relative">
                {student.name} {student.surname}{" "}
                <span
                  className="absolute text-gray-300 -top-2 pl-1"
                  style={{ fontSize: "10px" }}
                >
                  ({student.id})
                </span>
  ...
  return (
    <div className="baseContent p-4 rounded-md flex-1 m-4 mt-0">
      ...
      <div className="flex items-center justify-between">
        <h1 className="hidden md:block text-lg font-semibold">Tüm Veli Listesi</h1>
